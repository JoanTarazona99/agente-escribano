...
export default function Notebook() {
  ...
  // inline search
  const [inlineQuery, setInlineQuery] = useState("water dissociation recombination electromembrane bipolar membrane transport");
  ...
  useEffect(() => {
    if (job?.status === "completed") {
      setCurrentPage(1);
      setAllArticles([]);
      // Refresh notebook data (articles count) and article list
  ...
  const hasMore = !!articlesData?.next;
  ...
  return (
    <div className="nb-page">
      ...
      <div className="nb-workspace">
        {/* LEFT: Sources panel */}
        <aside className="nb-sources">
          <div className="nb-sources__header">
            <h2 className="nb-sources__title">{t("notebook.sources")}</h2>
          ...
          {/* ─── Inline search (siempre visible, estilo NotebookLM) ─── */}
          ...
          <div className="nb-sources__filters">
            ...
            <div className="nb-sources__filter-selects">
              ...
              <select
                className="nb-sources__select"
                onChange={(e) =>
                  updateFilters({ article_type: (e.target.value as never) || undefined })
                }
              >
                <option value="">{t("search.filter_all_types")}</option>
        ...
        {/* RIGHT: Detail panel */}
        <section className={`nb-detail${selectedArticleId ? " nb-detail--visible" : ""}`}>
          {selectedArticleId ? (
            <ArticleDetail
